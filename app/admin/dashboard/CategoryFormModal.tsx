...
export default function CategoryFormModal({ category, onClose, onSave }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ name: name.trim(), en: en.trim() || 'Category', emoji: emoji.trim() || '🐾', bg, textColor, border, imageUrl });
  };
      ...
      <div style={{ ...modalCardStyle, maxWidth: '460px' }} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '16px' }}>
          ...
          <label style={{ display: 'grid', gap: '6px', fontWeight: 700, fontSize: '13px' }}>
            카테고리명 (한글) *
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="예: 베드" style={inputStyle} />
          </label>

          <label style={{ display: 'grid', gap: '6px', fontWeight: 700, fontSize: '13px' }}>
            영문 라벨
            <input value={en} onChange={(e) => setEn(e.target.value)} placeholder="예: Bed" style={inputStyle} />
          </label>
